...
<body class="expanded skin-blue sidebar-mini">
	<div class="container">
		<div class="login-container">
			<div class="title">苏州智慧工会信息报送平台</div>
			<br>
			...
			<div class="login-box shadow">
				<form method="post" id="myform" action="${rc.contextPath}/user/login">
					...
					<div class="form-group">
                        <span class="glyphicon ico_uesr" aria-hidden="true"></span>
						<input type="text" name="accountId" class="form-control" value="${accountId}" placeholder="用户名" required> 
					</div>
					<div class="form-group">
                    	<span class="glyphicon ico_password" aria-hidden="true"></span>
						<input type="password" name="password" class="form-control" placeholder="密码" value="${password }" required> 
					</div>
					<div class="form-group">
                    	<span class="glyphicon ico_code" aria-hidden="true"></span>
						<input type="text" name="randPic" class="form-control" placeholder="验证码" value="" required>
						<span style="position:relative;top:-30px;left:65%;"><img src="${rc.contextPath }/user/randPic" onclick="this.src=this.src+'?';" title="刷新验证码"></span>
<%-- 						<span style="position:relative;top:-25px;left:78%;"><img src="${rc.contextPath }/user/randPic" onclick="this.src=this.src+'?';" title="刷新验证码"></span> --%>
					</div>
                	<div><c:if test="${not empty error}"><p class="code_tag">${error}</p></c:if></div>
					<input type="hidden" name="_token" value="mO1UFtAWHhPRrD6kwDCBqunqQ05N3LQhgOnEAZ4r">
					<button type="submit" class="btn btn-lg btn-success" id="login">立 即 登 录</button>
				</form>
			</div>
		</div>
	</div>
