...
<head>
	...
	$(function(){
		...
		$("#btn_add").on('click', function() {
			location.href = "${rc.contextPath}/SyncDesign/add";
		});
		<%-- 修改同步设定 --%>
		$("#btn_edit").click(function(){
			...
			if (ids.length == 0) {
				...
				//编辑自定义画面
				//alert(ids[0]);
				window.location.href = "${rc.contextPath}/SyncDesign/edit?no=" + ids[0];
			}
		});
		<%-- 同步设定发布按钮  --%>
		...
		$("#btn-publish-import").click(function() {
			...
			$.ajax({
				url:"${rc.contextPath}/SyncDesign/publish",
				type:"post",
				dataType:"json",
				data:{
					no:ids[0],
    ...
    <div class="content-wrapper">
        ...
        <div id="content" style="overflow-y:auto;">
            <div class="row">
                <!-- left column -->
                <!-- form start -->
                <form role="form" id="searchForm" method="post"  action="${rc.contextPath}/SyncDesign/list">
                	<div class="box_sreach clearfix">
                	<div class="fl">
	                  <label for="no" class="label-header label-header1">同步设定编码：</label>
	                  <input type="text" id="no" name="no" value="${no}" placeholder="同步设定编码：" class="form-control input-sm fl w15">
                  	</div>
                  	<div class="fl">
	                  <label for="name" class="label-header label-header1">同步设定名称：</label>
	                  <input type="text" id="name" name="name" value="${name}" placeholder="同步设定名称：" class="form-control input-sm fl w15">
                  	</div>
                   <div class="fl mr_30">
		          		<hq:auth act="SyncDesignList">
                        ...
                        <div class="box-body table-responsive no-padding">
                            <table class="table table-hover" id="dataTable">
				            	<tr>
						      	...
						      	<c:forEach items="${data.Design}" var="syncDesign">
						      	 <tbody>
						           <tr>
						           	 <td><input type="checkbox" value="${syncDesign.no }"></td>
						             <td>${syncDesign.no }</td>
						             <td>${syncDesign.name}</td>
						             <td>${syncDesign.model}</td>
						             <td>
						             	<c:if test="${syncDesign.status eq '0'}">保存</c:if>
						             	<c:if test="${syncDesign.status eq '1'}">发布</c:if>
						             </td>
						           </tr> 
						           </tbody>
						       </c:forEach>
		                                ...
		                                <tr>
		                                	...
		                                	<td>
												<select id="moduleNo" name="moduleNo" class="form-control input-sm fl w15" style="width:28%;">
													<option value=""></option>
													<c:forEach items="${data.Group }" var="group">
