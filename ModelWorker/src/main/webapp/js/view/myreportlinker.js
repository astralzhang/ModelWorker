(function($){
	var MyView = function(ele, appName, id, viewNo) {
		//alert(eval(headData));
		this.$element = ele;
	...
	MyView.prototype = {
		init : function() {
			...
			var objView = this;
			//获取数据
			var getData = function(id, viewNo) {
				//alert(appName + "/flowProcessor/view");
				MessageBox.processStart();
				$.ajax({
					url:appName + "/report/linkView",
					type:"post",
					dataType:"json",
					data:{
						id:id,
					...
					success:function(data) {
						...
						$("#myModalLabel1").html(data.viewName);
	...
	$.fn.myview = function(appName, id, viewNo) {
		var myview = new MyView(this, appName, id, viewNo);
		return myview.init();
	}	
})(jQuery);