(function(b){
		...
		path:{
			...
			props:{
				text:{
					...
					label:"显示",
					value:"",
					editor:function(){return new a.editors.textEditor()}
				}
			}
		},
		advpath:{
			...
			props:{
				...
				yesorno:{
					...
					label:"符合",
					value:"Y",
					editor:function(){return new a.editors.selectEditor([{name:"Yes",value:"Y"},{name:"No",value:"N"}]);}
				}
			}
		},
		tools:{
			attr:{left:232,top:98},
			pointer:{},
			path:{},
			states:{},
	...
	a.util={
		isLine:function(g,f,e){
			var d,c;
			if((g.x-e.x)==0){d=1}else{d=(g.y-e.y)/(g.x-e.x)}
			c=(f.x-e.x)*d+e.y;
			if((f.y-c)<10&&(f.y-c)>-10){f.y=c;return true}
			return false
		},
		center:function(d,c){
	...
	a.rect=function(p,m){
		...
		var l=function(){t.attr({opacity:1});e.attr({opacity:1});f.attr({opacity:1})};
		...
		function w(){
			s.show();
			for(var o in i){i[o].show()}
		}
		function d(){
			s.hide();
			for(var o in i){i[o].hide()}
		}
		function B(){
		...
		this.toJson=function()
		{
			...
			for(var o in E.props){
				//if (E.props[o].value != null && E.props[o].value != "" && E.props[o].value.length > 0 && E.props[o].value.charAt(0) == "{") {
				//	r+=o+":{condition:"+E.props[o].value+"},";
				//} else {
		...
		a.path=function(q,n,u,e){
			var v=this,z=n;
			var B=null;
			if (u.getType() == "condition") {
				//起始节点为条件时
				B = b.extend(true,{},a.config.advpath);
			} else {
				B = b.extend(true,{},a.config.path);
			}
			var i,t,f,h=B.textPos,y,w,k=u,s=e,g="path"+a.util.nextId(),x;
			function p(G,H,D,L){
				...
				this.type=function(P){
					if(P){M=P}else{return M}
				};
				this.node=function(P){
					if(P){r=P}else{return r}
				};
				this.left=function(P){
					if(P){o=P}else{return o}
				};
			...
			a.props=function(h,f){
				var j=this,
					c=b("#myflow_props").hide().draggable({handle:"#myflow_props_handle"}).resizable().css(a.config.props.attr).bind("click",function(){return false}),
					e=c.find("table"),
					g=f,i;
			...
			a.init=function(x,r){
				...
				if(a.config.editable){
					...
					b("#myflow_tools .node").hover(function(){b(this).addClass("mover");},
							function(){b(this).removeClass("mover");});
					b("#myflow_tools .selectable").click(function(){
						b(".selected").removeClass("selected");
						b(this).addClass("selected");
						b(y).data("mod",this.id);
					});
					b("#myflow_tools .state").each(function(){b(this).draggable({helper:"clone"});});
					b(x).droppable({accept:".state",drop:function(c,i){
						b(y).trigger("addrect",[i.helper.attr("type"),{attr:{x:i.helper.offset().left-235,y:i.helper.offset().top-125}}]);
					}
					});
					//保存按钮点击
			...
			b.fn.myflow=function(c){
				return this.each(function(){a.init(this,c);});
			};
			b.myflow=a;
		})(jQuery);